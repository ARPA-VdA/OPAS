            ...
            try:
                ...
                if data:
                    ...
                    if data == b'T CO':
                        ...
                        connection.sendall('T CO={}UGM'.format(v).encode())
                    ...
                    # diags
                    elif data == b'T COMEAS':
                        print ('sending co meas value')
                        v = random.uniform(40, 55)
                        if v % 2:
                            connection.sendall('T CO MEAS={0} MV'.format(v).encode())
                        else:
                            connection.sendall('T CO MEAS={0} CNT'.format(v).encode())
                    elif data == b'T COREF':
                        print ('sending co ref value')
                        v = random.uniform(40, 55)
                        if v % 2:
                            connection.sendall('T CO REF={0} MV'.format(v).encode())
                        else:
                            connection.sendall('T CO REF={0} CNT'.format(v).encode())
                    elif data == b'T BENCHTEMP':
                        print ('sending bench tmp value')
                        v = random.uniform(40, 55)
                        connection.sendall('T BENCH TEMP={0} C'.format(v).encode())
                    elif data == b'T WHEELTEMP':
                        print ('sending wheel tmp value')
                        v = random.uniform(40, 55)
                        connection.sendall('T WHEEL TEMP={0} C'.format(v).encode())
                    elif data == b'T BOXTEMP':
                        print ('sending box tmp value')
                        v = random.uniform(40, 55)
                        connection.sendall('T BOX TEMP={0} C'.format(v).encode())
                    elif data == b'T MRRATIO':
                        print ('sending mr ratio value')
                        v = random.uniform(40, 55)
                        connection.sendall('T MR RATIO={0}'.format(v).encode())
                    elif data == b'T COSLOPE':
                        print ('sending co slope value')
                        v = random.uniform(40, 55)
                        connection.sendall('T SLOPE={0}'.format(v).encode())
                    elif data == b'T COOFFSET':
                        print ('sending co offset value')
                        v = random.uniform(40, 55)
                        connection.sendall('T OFFSET={0}'.format(v).encode())
                        ...
                        connection.sendall('T SAMPLE TEMP={0} C'.format(v).encode())
                    elif data == b'T PHOTOTEMP':
                        print ('sending photo temp value')
                        v = random.uniform(-0, -5)
                        connection.sendall('T PHT DRIVE={0} MV'.format(v).encode())
