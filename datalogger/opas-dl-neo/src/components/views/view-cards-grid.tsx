...
import { formatValue } from "@/components/instruments/readings-section"
            ...
            <CardContent className="px-3 pb-3 pt-0 flex-1 flex flex-col gap-1.5">
              {moduleChannels.map(ch => {
                ...
                return (
                  <div
                    key={ch.channel.id}
                    className={`flex items-center justify-between gap-2 rounded-md bg-muted/20 px-2 py-1.5 ${!ch.channel.active ? 'opacity-50' : ''}`}
                  >
