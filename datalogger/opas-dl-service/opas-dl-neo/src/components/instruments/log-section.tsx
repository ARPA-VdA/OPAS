...
export function LogSection({ driverId, instrumentName }: LogSectionProps) {
  ...
  const fetchLines = useCallback(async () => {
    if (driverId) {
      const result = await window.electron.getDriverLogLines(driverId)
      if (!result) return
      setLines(prev => mergeLogLines(prev, result, pinnedRef.current, LOG_BUFFER))
    } else {
      const result = await window.electron.getLogLines()
      if (!result) return
      const filtered = result.filter(l => l.toLowerCase().includes(instrumentName.toLowerCase()))
      setLines(prev => mergeLogLines(prev, filtered, pinnedRef.current, LOG_BUFFER))
    }
  }, [driverId, instrumentName])

  // driverId/instrumentName changing means the log view now points at a
  // different instrument - the accumulated buffer belongs to the old one and
  // must not be merged into the new one's lines.
  useEffect(() => {
    setLines([])
    setPinned(true)
    fetchLines()
  ...
  return (
    <div className="relative rounded-md overflow-hidden border border-zinc-300 dark:border-zinc-800 flex flex-col min-h-[200px] max-h-[400px]">
      <div className="flex items-center justify-between bg-zinc-200 dark:bg-zinc-900 px-3 py-1.5 border-b border-zinc-300 dark:border-zinc-800">
        <span className="text-xs text-zinc-600 dark:text-zinc-400 font-mono">{instrumentName} · log</span>
        <Button
      ...
      <ContextMenu>
        <ContextMenuTrigger asChild>
          ...
          >
            {lines.length === 0
              ? <span className="text-zinc-500 dark:text-zinc-600">Nessun log per questo strumento</span>
              : lines.map((line, i) => (
