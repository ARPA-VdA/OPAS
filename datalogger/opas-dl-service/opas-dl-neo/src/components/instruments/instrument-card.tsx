...
// True when a card would show the amber "connection problem" treatment:
// either the driver process isn't running at all, or it's running but the
// instrument is reporting a missing reading (P.COD 128). Does NOT account
// for the instrument being deliberately disabled (moduleConfig.active ===
// false) - callers that aggregate across instruments (e.g. the dashboard
// alarms banner) must filter disabled instruments out themselves, the same
// way this card's own opacity/grayscale treatment does visually.
export function instrumentNotResponding(instrument: MergedInstrument): boolean {
  return !instrument.alive || (instrument.alive && instrument.isMissing)
}
...
export function InstrumentCard({ instrument, onClick, onAction, onToggleActive }: InstrumentCardProps) {
  ...
  const { id, alive, model, brand, moduleConfig, lastModified,
          pid, uptimeSeconds, startTime, driverKey, driverExists,
  ...
  const notConnected = instrumentNotResponding(instrument)
