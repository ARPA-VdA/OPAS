...
export function getUiDocsPath(lang: 'en' | 'it') {
    // Documentation of this app (opas-dl-neo) itself, not the Python service.
    // Shipped as markdown alongside the app (extraResources, see
    // electron-builder.json) rather than baked into the JS bundle - plain
    // markdown is easier to write/review/translate than JS/TSX, though
    // updating it still means a new release like the rest of the UI.
    return path.join(
        app.getAppPath(),
        isDev() ? '.' : '..',
        'docs', 'ui', lang
