...
import { getPreloadPath, getLogPath, getDriverLogPath, getOpasConfigPath, getOutputPath, getOpasNeoDataDir, getInstantFilePath, getServiceDocsPath, getUiDocsPath, getSplashImagePath, getDriversPath } from './pathResolver.js';
import { getSettings, saveSettings } from './configManager.js';
import { listViews, getViewById, createView, updateView, deleteView, setFavoriteView } from './viewsManager.js';
...
// Shared by getServiceDocs and getUiDocs: both read a flat folder of .md files.
function readDocsDir(docsDir: string): ServiceDocFile[] {
    try {
...
app.on("ready", () => {
    ...
    ipcMain.handle("exportSettings", async () => {
        const result = await dialog.showSaveDialog(mainWindow, {
            ...
            defaultPath: 'opas-dl-neo-settings.json',
            buttonLabel: 'Esporta',
            filters: [{ name: 'Impostazioni JSON', extensions: ['json'] }],
        });
        if (result.canceled || !result.filePath) return { success: false, canceled: true };
        try {
            fs.writeFileSync(result.filePath, JSON.stringify(getSettings(), null, 2), 'utf-8');
            return { success: true };
        } catch (err) {
            console.error('[main] Failed to export settings:', err);
    ...
    ipcMain.handle("importSettings", async () => {
        const result = await dialog.showOpenDialog(mainWindow, {
            properties: ['openFile'],
            title: 'Seleziona file di impostazioni',
            buttonLabel: 'Importa',
            filters: [{ name: 'Impostazioni JSON', extensions: ['json'] }],
        });
        ...
        try {
            ...
            if (typeof parsed.opasDlPath === 'string') patch.opasDlPath = parsed.opasDlPath;
            if (typeof parsed.rawValueMaxDecimals === 'number') patch.rawValueMaxDecimals = parsed.rawValueMaxDecimals;

            saveSettings(patch);
            return { success: true, settings: getSettings() };
        } catch (err) {
            console.error('[main] Failed to import settings:', err);
            return { success: false };
        }
    });
    ...
    ipcMain.handle("setFavoriteView", (_: any, id: string, favorite: boolean) => {
        return setFavoriteView(id, favorite);
    });
    ...
    ipcMain.handle("openGraphsWindow", (_: any, panel: any) => {
        // Fully independent window (no `parent`) - stays open regardless of
        // what happens to mainWindow or any other detached graph window, and
        // isn't forced to stay above them. Loads the exact same renderer
        // bundle as mainWindow; App.tsx branches on ?window=graphs to render
        // just this one panel instead of the full app shell.
        ...
        graphsWindow.once('ready-to-show', () => graphsWindow.show());
        const query = `window=graphs&panel=${encodeURIComponent(JSON.stringify(panel))}`;
        if (isDev()) {
            graphsWindow.loadURL(`http://localhost:5123/?${query}`);
        } else {
            graphsWindow.loadFile(path.join(app.getAppPath(), '/dist-react/index.html'), { search: query });
        }
    });
    ipcMain.handle("getServiceDocs", (_: any, lang: string) => {
        ...
        return readDocsDir(docsDir);
    });
    ipcMain.handle("getUiDocs", (_: any, lang: string) => {
        const requestedLang = lang === 'it' ? 'it' : 'en';
        let docsDir = getUiDocsPath(requestedLang);
        if (!fs.existsSync(docsDir) || fs.readdirSync(docsDir).length === 0) {
            docsDir = getUiDocsPath('en');
        }
        return readDocsDir(docsDir);
    });
