...
type ViewEntry = {
    ...
    channelCount: number;
    lastModified: string;
    // At most one view has this true at a time - see setFavoriteView, which
    // enforces the single-favorite invariant server-side.
    favorite: boolean;
};
...
type EventPayloadMapping = {
    ...
    getUiDocs: ServiceDocFile[];
    exportSettings: { success: boolean; canceled?: boolean } | null;
    importSettings: { success: boolean; settings?: AppConfig } | null;
    listViews: ViewEntry[];
    ...
    setFavoriteView: { success: boolean } | null;
};
...
interface Window {
    electron: {
        ...
        exportSettings: () => Promise<{ success: boolean; canceled?: boolean } | null>;
        importSettings: () => Promise<{ success: boolean; settings?: AppConfig } | null>;
        ...
        openGraphsWindow: (panel: {
            seriesDefs: { id: string; instrumentName: string; channelName: string; colorIndex: number; pollingInterval: number }[];
            kind: HistoryKind;
            fromDateTime: string;
            toDateTime: string;
            live: boolean;
        ...
        getUiDocs: (lang: string) => Promise<ServiceDocFile[]>;
        ...
        setFavoriteView: (id: string, favorite: boolean) => Promise<{ success: boolean } | null>;
    }
}