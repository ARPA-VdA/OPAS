...
class APIDriver(driver_sdk.BaseDriver):
    ...
    def _get_instrument_status(self) -> float | None:
        ...
        try:
            # Status query - adapt as needed based on actual protocol
            # For now, return None until specific status command is known
            return None
        ...
        try:
            ...
            if channels:
                ...
                if param_id == 1126 or (last_name and last_name.lower() == "status"):
                    ...
                    # Add status as additional value to avoid overwriting last channel
                    # This maintains compatibility with VB.NET behavior
                    pass
