...
extra or missing column breaks ingestion, not just cosmetics.
    ...
    file_istantanei_raw\\<FILE-HEADER>.dat                    DATA+ORA,ID,VAL,P.COD (VAL = dato grezzo,
                                                               pre-Formule; vedi output_manager.Reading.raw_value).
                                                               NON fa parte del contratto di rete con il centro
                                                               (docs/formato-opas-v2.txt) - consumato solo dal
                                                               client Electron per mostrare il valore raw in UI.
...
def _status_code_for_value(value: float | None) -> int:
    ...
    """
    return 128 if value is None else 0  # misura non valida / valido
...
class _HourBucket:
    ...
    def compute(self):
        ...
        s_cod = 0  # nessun segnale reale di stato stazione esiste oggi
