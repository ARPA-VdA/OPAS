...
class RuntimePaths:
    ...
    def get(self, key: str, default=None):
        """Get a path by key, with optional default."""
        return self._paths_dict.get(key, default)


# ========== Backward Compatibility Functions ==========

def get_runtime_root(base_dir=None) -> Path:
    """
    Return the actual runtime root directory.
    
    Backward compatibility wrapper around RuntimePaths singleton.
    """
    logging.debug(f"[RuntimePaths] get_runtime_root called with base_dir={base_dir}")
    root = RuntimePaths(base_dir).base_dir
    logging.debug(f"[RuntimePaths] get_runtime_root returning: {root}")
    return root


def load_folder_config(base_dir: Path = None) -> dict:
    """
    Load folder configuration from folder_config.json.

    Backward compatibility wrapper around RuntimePaths singleton.
    Returns a dictionary with absolute paths computed from relative config paths.
    All values are Path objects.
    """
    logging.debug(f"[RuntimePaths] load_folder_config called with base_dir={base_dir}")
    config = RuntimePaths(base_dir).to_dict()
    logging.debug(f"[RuntimePaths] load_folder_config returning {len(config)} paths")
    return config
