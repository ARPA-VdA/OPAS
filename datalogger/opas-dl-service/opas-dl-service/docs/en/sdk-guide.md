...
- **Automatic reconnect via a watchdog, not `connect()` retries.** If
  `connect()` raises, the driver keeps running and reports missing values
  every cycle (see step 7 below) exactly as before — `connect()` itself is
  still only ever called once "manually". But `run_driver()` separately
  tracks whether *any* active channel has produced a real (non-missing) raw
  reading recently; if not, for `AutoReconnectSeconds` seconds (module
  config field, default `60`, `0` disables it), it calls `disconnect()`
  then `connect()` again on the same instance and resumes polling (see
  step 8 below). This also recovers a driver whose very first `connect()`
  call failed. A driver that wants finer-grained retry semantics of its
  own can still implement them, typically lazily inside
  `read_channel()`/`read_all_channels()` — the watchdog is purely additive
  and doesn't change per-cycle read failure handling.
...
7. **Call `connect()` once, and start the watchdog clock.** If it raises,
   `connected` stays `False` until the watchdog (step 8) reconnects it —
   see the "automatic reconnect via a watchdog" note above. Either way, a
   `last_data_at` timestamp is set here so the watchdog's first check is a
   full `AutoReconnectSeconds` away, not instant.
8. **The polling loop**, while `common.should_run()` is `True`:
   ...
   - **Watchdog check** (only if `AutoReconnectSeconds` > 0 and there's at
     least one active channel): if this cycle produced any real
     (non-missing) raw reading, reset the "last data seen" clock.
     Otherwise, once that clock has been stale for `AutoReconnectSeconds`,
     log a warning, call `disconnect()` then `connect()` again (each
     wrapped in try/except, same as elsewhere), update `connected`
     accordingly, and reset the clock so the next attempt is also at
     least `AutoReconnectSeconds` away.
