...
- **Riconnessione automatica tramite un watchdog, non tramite retry di
  `connect()`.** Se `connect()` solleva un'eccezione, il driver continua a
  girare riportando valori mancanti a ogni ciclo (vedere punto 7 più
  sotto) esattamente come prima — `connect()` in sé viene comunque
  chiamata una sola volta "manualmente". Ma `run_driver()` tiene traccia
  separatamente di se *un qualsiasi* canale attivo ha prodotto una
  lettura grezza reale (non mancante) di recente; se per
  `AutoReconnectSeconds` secondi (campo di config del modulo, default
  `60`, `0` lo disabilita) non ne ha prodotta nessuna, chiama
  `disconnect()` e poi `connect()` di nuovo sulla stessa istanza e
  riprende il polling (vedere punto 8 più sotto). Questo recupera anche
  un driver la cui *prima* chiamata a `connect()` è fallita. Un driver
  che vuole una logica di retry più fine può comunque implementarla da
  sé, tipicamente in modo lazy dentro
  `read_channel()`/`read_all_channels()` — il watchdog è puramente
  additivo e non cambia la gestione dei fallimenti di lettura per-ciclo.
...
7. **Chiama `connect()` una volta, e avvia l'orologio del watchdog.** Se
   solleva un'eccezione, `connected` resta `False` finché il watchdog
   (punto 8) non la riconnette — vedere la nota "riconnessione automatica
   tramite un watchdog" sopra. In entrambi i casi, qui viene impostato un
   timestamp `last_data_at` così il primo controllo del watchdog avviene
   dopo un intero `AutoReconnectSeconds`, non subito.
8. **Il loop di polling**, finché `common.should_run()` è `True`:
   ...
   - **Controllo del watchdog** (solo se `AutoReconnectSeconds` > 0 e c'è
     almeno un canale attivo): se questo ciclo ha prodotto una lettura
     grezza reale (non mancante), l'orologio "ultimo dato visto" viene
     azzerato. Altrimenti, una volta che l'orologio è rimasto fermo per
     `AutoReconnectSeconds`, viene loggato un warning, chiamato
     `disconnect()` e poi `connect()` di nuovo (ognuno avvolto in
     try/except, come altrove), `connected` viene aggiornato di
     conseguenza, e l'orologio viene azzerato così anche il prossimo
     tentativo sarà ad almeno `AutoReconnectSeconds` di distanza.
