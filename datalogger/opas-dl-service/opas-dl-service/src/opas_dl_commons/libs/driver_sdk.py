...
class BaseDriver(ABC):
    ...
    run_driver() also runs a watchdog: if no channel has produced a real
    (non-missing) reading for AutoReconnectSeconds seconds - including the
    case where the initial connect() above failed and never recovered - it
    calls disconnect() then connect() again on this same instance and keeps
    polling. See AutoReconnectSeconds in run_driver()'s docstring for the
    exact semantics; set it to 0 to disable this and implement your own
    retry semantics instead, e.g. lazily inside
...
def run_driver(
    ...
    *,
    ...
    module_config["AutoReconnectSeconds"] (default 60; 0 or negative disables
    it) controls a watchdog: if this module has at least one active channel
    and none of them has produced a non-missing raw reading (the value
    before Formule, i.e. what ends up in Reading.raw_value) for this many
    seconds, the loop calls instance.disconnect() then instance.connect()
    again in place - same process, same instance - and resumes polling,
    exactly as if this were the very first connect() attempt. This also
    recovers a driver whose *initial* connect() failed, which previously
    never retried for the rest of the process's life. Exceptions from either
    call are caught and logged, same as the startup connect() and the
    shutdown disconnect(). The watchdog timer starts right after the initial
    connect() attempt, so the first check is not gated by data that could
    not possibly exist yet, and it resets every time real data is seen, so a
    genuinely healthy driver never sees an extra disconnect()/connect() pair.
    """
    ...
    try:
        auto_reconnect_seconds = int(module_config.get("AutoReconnectSeconds", 60))
    except (TypeError, ValueError):
        auto_reconnect_seconds = 60
    # Nothing to watch for if there's nothing to poll - an empty
    # active_channels would otherwise never see "real data" and the
    # watchdog would fire every interval forever for no reason.
    auto_reconnect_enabled = auto_reconnect_seconds > 0 and bool(active_channels)
    ...
    # Started here (not at the top of the function) so the first watchdog
    # check is gated by a full AutoReconnectSeconds from process start,
    # whether or not the connect() above succeeded - not an instant retry.
        ...
        while common.should_run() and (max_cycles is None or cycles < max_cycles):
            ...
            for ch in active_channels:
                ...
                if connected:
                    ...
                    raw_value = value  # raw value as read from the instrument, before Formule
                else:
                    value = None
                    raw_value = None
            ...
            if auto_reconnect_enabled:
                if any_real_data:
                    last_data_at = time.monotonic()
                elif time.monotonic() - last_data_at >= auto_reconnect_seconds:
                    log.warning(
                        f"No data from the instrument for >= {auto_reconnect_seconds}s; reconnecting"
                    )
                    try:
                        instance.disconnect()
                    except Exception as e:
                        log.warning(f"disconnect() failed during auto-reconnect: {e}")
                    try:
                        instance.connect()
                        connected = True
                        log.warning("Auto-reconnect succeeded")
                    except Exception as e:
                        connected = False
                        log.warning(f"Auto-reconnect connect() failed: {e}")
                    last_data_at = time.monotonic()
