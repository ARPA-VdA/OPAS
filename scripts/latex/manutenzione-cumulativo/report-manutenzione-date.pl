...
#------------------------------------------------------------------------------
#
#   Script Name : report-manutenzione-date.pl
#        Author : Ecometer s.n.c.
#          Date : 2024-09-30
#
#   REPORT MANUTENZIONE CUMULATIVO
...
#       (user_id)
#
#       $ perl /var/www/bobo/public/report-manutenzione-date.pl 2022-01-01 2022-12-31 1 1000 16
#
#------------------------------------------------------------------------------
#------------------------------------------------------------------------------
...
my $title    = 'Reports';
my $location = 'Opas';
my $logfile  = 'report-manutenzione-date.log';
my $version  = '1.0';
...
if ($num_args == 5) {
    ...
    $log->warn('Wrong arguments provided {date1, date2, net, st, user}, sample: report-manutenzione-date.pl "2022-01-01 00:00:00" "2022-01-31 23:59:59" -1 -1 1');
    end();
    exit(1);
}
# log
...
my $tex_file_wrapper = $working_path.'/report_manutenzione_date_'.$random_str.'.tex';
my $tex_file_body = $working_path.'/manutenzione_date_'.$random_str.'.tex';
my $tex_file_headfoot = $working_path.'/header_footer_manutenzione_date_'.$random_str.'.tex';
...
my $sql_main = qq{
    SELECT DISTINCT ON (ma_id)
        ...
        TO_CHAR(vm.maintenance_fulldate:: date, 'DD/MM/YYYY') AS date,
        TO_CHAR(vm.maintenance_fulldate:: time, 'HH24:MI') AS time_format,
        TO_CHAR(vm.maintenance_fulldate, 'DD-MM-YYYY HH24:MI') AS maintenance_fulldate_formatted,
        COALESCE(vm.maintenance_note, 'Nessuna nota') AS maintenance_note
    ...
    LEFT JOIN bobo.view_users vu ON (vm.us_id = vu.user_id)
    WHERE
        vm.maintenance_fulldate BETWEEN '$date_start'::timestamp AND '$date_end'::timestamp
...
if($station != -1){
    $sql_main .= qq{
        AND vm.station_id = $station
    };
}

# query end
$sql_main .= qq {
    ORDER BY ma_id, maintenance_fulldate
    -- LIMIT 5 -- only test
};

#------------------------------------------------------------------------------
# GET RECORD MANUTENZIONI
#------------------------------------------------------------------------------
my $maintenances = dbh_get_rows_arrayref( $sql_main );
$log->info(Dumper($maintenances));

# controllo se il risultato della query è vuoto
my $rows = scalar @{$maintenances};
if ($rows == 0){
...
# get number of maintenances
my $num_manutenzioni =  scalar @{$maintenances};
$log->info("LUNGHEZZA ARRAY MANUTENZIONI: $num_manutenzioni");

# flag per identificare il primo valore dell'array delle manutenzioni (utile per creare una sola volta il file header_footer)
my $first = 0;

# loop dei report manutenzioni
foreach my $maintenance ( @{$maintenances} )
{
    ...
    if ($first == 0) {
        ...
        $headfoot .= '    \quad'."\n";
        $headfoot .= '    \raisebox{2ex}{\includegraphics[height=1cm, keepaspectratio]{'.$logo_path.''.$maintenance->{station_network_type_logo}.'}}'."\n";
        $headfoot .= '}'."\n";
        $headfoot .= "\n";
        $headfoot .= '\fancyhead[R]{'."\n";
        $headfoot .= '    \raisebox{1.5ex}{\textcolor{greyHEADFOOT}{\bfseries{\LARGE '.latex_encode($maintenance->{station_network_type_desc}).'}}} \\\\'."\n";
        $headfoot .= '}'."\n";
        $headfoot .= '%%----------------- FOOTER -----------------'."\n";
        $headfoot .= '\fancyfoot[L]{'."\n";
        $headfoot .= '  \textcolor{greyHEADFOOT}{\bfseries{'.$foot_desc.' - '.latex_encode($maintenance->{station_network_type_desc}).'}} \\\\'."\n";
    ...
    my $reportid = $maintenance->{ma_id};
    $log->info("report_id: $reportid");
    ...
    #------------------------------------------------------------------------------
    # GET RECORD OPERAZIONI
    #------------------------------------------------------------------------------
    my $operations = dbh_get_rows_arrayref( $sql_op );
    $log->info(Dumper($operations));

    #------------------------------------------------------------------------------
    # CORPO CENTRALE
    #------------------------------------------------------------------------------
    $log->debug('corpo centrale manutenzione cumulativo ...');

    # costruzione del file .tex
    # titolo
    $body .= '\begin{center}'."\n";
    $body .= '    \hypertarget{Manutenzione_'.$reportid.'}{}'."\n";
    $body .= '    \textcolor{azureCF}{\Large{Report Manutenzione n. \textbf{'.$maintenance->{ma_id}.'} del \textbf{'.$maintenance->{date}.'} alle \textbf{'.$maintenance->{time_format}.'}}} % TITOLO'."\n";
    $body .= '\end{center}'."\n";
    ...
    $body .= '        >{\raggedright\arraybackslash}X'."\n";
    $body .= '    }'."\n";
    $body .= '        \bfseries{Operatore:} & \multicolumn{2}{p{6cm}}{\mdseries{'.controlnull(latex_encode($maintenance->{user_fullname})).'}} & \bfseries{Provincia:} & \mdseries{'.controlnull(latex_encode($maintenance->{province_name})).'} & \\\\'."\n";
    $body .= '        \bfseries{Azienda:} & \multicolumn{2}{p{5cm}}{\mdseries{'.controlnull(latex_encode($maintenance->{company_name})).'}} & \bfseries{Stazione:} & \multicolumn{2}{p{6cm}}{\mdseries{'.controlnull(latex_encode($maintenance->{station_name})).'}} \\\\'."\n";
    $body .= '        \bfseries{Note:} & \multicolumn{5}{p{40em}}{\mdseries{'.controlnull(latex_encode($maintenance->{maintenance_note})).'}} \\\\'."\n";
    $body .= '    \end{tabularx}'."\n";
    $body .= '\end{center}'."\n";
    $body .= "\n";
    $body .= '\vspace{0.5cm}'."\n";
    ...
    # corpo tabelle
    $body .= '\textcolor{BOBOblue}{\bfseries{\large Operazioni svolte}}'."\n";
    $body .= '\begin{center}'."\n";
    $body .= '    \setlength\LTleft{0cm}'."\n";
    $body .= '    \renewcommand{\arraystretch}{1.75}'."\n";
    $body .= '    \begin{longtable}[c]{ p{40mm} p{25mm} p{25mm} p{20mm} p{25mm} p{35mm} }'."\n";
    $body .= '        \bfseries{Strumento} & \bfseries{Operazione} & \bfseries{Frequenza} & \bfseries{Taratura} & \bfseries{Scad. filtri} & \bfseries{Note} \\\\'."\n";
    $body .= '        \arrayrulecolor{lines}\hline'."\n";
    $body .= '        \arrayrulecolor{lines}\hline'."\n";
    $body .= '        \endfirsthead'."\n";
    $body .= "\n";
    $body .= '        \multicolumn{2}{l}{\textcolor{BOBOblue}{\mdseries{\textit{Continua dalla pagina precedente...}}}} & & & & \\\\'."\n";
    $body .= '        \bfseries{Strumento} & \bfseries{Operazione} & \bfseries{Frequenza} & \bfseries{Taratura} & \bfseries{Scad. filtri} & \bfseries{Note} \\\\'."\n";
    $body .= '        \arrayrulecolor{lines}\hline'."\n";
    $body .= '        \arrayrulecolor{lines}\hline'."\n";
    $body .= '        \endhead'."\n";
    $body .= "\n";
    $body .= '        & & & & \multicolumn{2}{r}{\textcolor{BOBOblue}{\mdseries{\textit{Continua nella prossima pagina...}}}} \\\\'."\n";
    $body .= '        \endfoot'."\n";
    $body .= "\n";
    $body .= '        \arrayrulecolor{lines}\hline'."\n";
    $body .= '        \arrayrulecolor{lines}\hline'."\n";
    ...
    # array delle tarature
    my @calib_ids = ();

    # loop per popolare la tebella delle operazioni
    foreach my $operation ( @{$operations} )
    {
        $log->info("calib_id:" .$operation->{calib_id});
        if (defined $operation->{calib_id}) {
            push @calib_ids, $operation->{calib_id};
        }

        # costruzione del nome completo dello strumento
        my $instrument_fullname = $operation->{instr_type_fullname};
        if (defined $operation->{instr_name} && $operation->{instr_name} ne ''){
            $instrument_fullname .= ' ('.$operation->{instr_name}.')';
        }

        if (!$operation->{calib_id}){
            $body .= '        \mdseries{'.controlnull(latex_encode($instrument_fullname)).'} & \mdseries{'.controlnull(latex_encode($operation->{operation_description})).'} & \mdseries{'.controlnull(latex_encode($operation->{frequency_desc})).'} & \mdseries{'.controlnull(latex_encode($operation->{calib_id})).'} & \mdseries{'.controlnull(latex_encode($operation->{main_operation_filters_exp})).'} & \mdseries{'.controlnull(latex_encode($operation->{main_operation_note})).'} \\\\'."\n";
        } else {
            $body .= '        \mdseries{'.controlnull(latex_encode($instrument_fullname)).'} & \mdseries{'.controlnull(latex_encode($operation->{operation_description})).'} & \mdseries{'.controlnull(latex_encode($operation->{frequency_desc})).'} & \hyperlink{TARATURA_'.$operation->{calib_id}.'}{\underline{\mdseries{'.latex_encode($operation->{calib_id}).'}}} & \mdseries{'.controlnull(latex_encode($operation->{main_operation_filters_exp})).'} & \mdseries{'.controlnull(latex_encode($operation->{main_operation_note})).'} \\\\'."\n";
        }
        $body .= '        \arrayrulecolor{lines}\hline'."\n";
    }
    ...
    # get element postion (maintenences)
    # $maintenance ( @{$maintenances} )
    my $maintenance_idx = firstidx { $_ eq $maintenance } @{$maintenances};
    $maintenance_idx++; # 0 excluded
    $log->info("POSIZIONE MANUTENZIONE: $maintenance_idx");

    # get number of calibrations
    my $num_tarature =  scalar @calib_ids;
    $log->info("LUNGHEZZA ARRAY TARATURE: $num_tarature");

    # check if there are any calibration
    if ($num_tarature == 0){
        $body .= '\newpage'."\n"; # no signature
    } else {
        # inserimento tarature
        foreach my $calibid ( @calib_ids ) {
            #------------------------------------------------------------------------------
            # QUERY TARATURA
            #------------------------------------------------------------------------------
            my $sql_cal = qq{
            ...
            # GET RECORD TARATURA
            #------------------------------------------------------------------------------
            my $calibration = dbh_get_row_hashref( $sql_cal );
            $log->info(Dumper($calibration));
            ...
            $body .= '    \textcolor{azureCF}{\Large{Report Taratura n. \textbf{'.$calibration->{calib_id}.'} del \textbf{'.$calibration->{date}.'} alle \textbf{'.$calibration->{time_format}.'}}} % TITOLO'."\n";
            $body .= '\end{center}'."\n";
            ...
            $body .= '        >{\raggedright\arraybackslash}X'."\n";
            $body .= '    }'."\n";
            $body .= '        \bfseries{Strumento:} & \multicolumn{5}{p{40em}}{\mdseries{'.controlnull(latex_encode($instrument_fullname)).' (cat. \bfseries{'.controlnull(latex_encode($calibration->{category_name})).'})}} \\\\'."\n";
            ...
            given ($calibration->{category_id}) {
                when (1) {
                    ...
                    if ($calibration->{calib_multipoint}) {
                        ...
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        # $body .= '        \multicolumn{2}{l}{\multirow{2}*{\textcolor{azureCF}{\bfseries{\large L1}}}} & \mdseries{'.controlnull($calib_json->{'l1-read-span-so2'}).'} & \mdseries{'.controlnull($calib_json->{'l1-theory-span-so2'}).'} \\\\'."\n";
                        $body .= '        \multicolumn{2}{l}{\textcolor{azureCF}{\bfseries{\large L1}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l1-read-span-so2'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l1-theory-span-so2'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multicolumn{2}{l}{\textcolor{azureCF}{\bfseries{\large L2}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l2-read-span-so2'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l2-theory-span-so2'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multicolumn{2}{l}{\textcolor{azureCF}{\bfseries{\large L3}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l3-read-span-so2'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l3-theory-span-so2'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multicolumn{2}{l}{\textcolor{azureCF}{\bfseries{\large L4}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l4-read-span-so2'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l4-theory-span-so2'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multicolumn{2}{l}{\textcolor{azureCF}{\bfseries{\large L5}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l5-read-span-so2'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l5-theory-span-so2'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '    \end{tabularx}'."\n";
                        $body .= '\end{center}'."\n";
                    } else {
                ...
                when (2) {
                    ...
                    if ($calibration->{calib_multipoint}) {
                        ...
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multirow{2}*{\textcolor{azureCF}{\bfseries{\large L1}}} & \bfseries{Span letto:} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l1-read-nox-span-noxnono2'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l1-read-no-span-noxnono2'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        ...
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multirow{2}*{\textcolor{azureCF}{\bfseries{\large L2}}} & \bfseries{Span letto:} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l2-read-nox-span-noxnono2'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l2-read-no-span-noxnono2'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        ...
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multirow{2}*{\textcolor{azureCF}{\bfseries{\large L3}}} & \bfseries{Span letto:} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l3-read-nox-span-noxnono2'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l3-read-no-span-noxnono2'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        ...
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multirow{2}*{\textcolor{azureCF}{\bfseries{\large L4}}} & \bfseries{Span letto:} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l4-read-nox-span-noxnono2'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l4-read-no-span-noxnono2'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                ...
                when (3) {
                    ...
                    if ($calibration->{calib_multipoint}) {
                        ...
                        $body .= '         &  & \bfseries{Span letto} & \bfseries{Span teorico} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        # $body .= '        \multicolumn{2}{l}{\multirow{2}*{\textcolor{azureCF}{\bfseries{\large L1}}}} & \mdseries{'.controlnull($calib_json->{'l1-read-span-co'}).'} & \mdseries{'.controlnull($calib_json->{'l1-theory-span-co'}).'} \\\\'."\n";
                        $body .= '        \multicolumn{2}{l}{\textcolor{azureCF}{\bfseries{\large L1}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l1-read-span-co'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l1-theory-span-co'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multicolumn{2}{l}{\textcolor{azureCF}{\bfseries{\large L2}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l2-read-span-co'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l2-theory-span-co'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multicolumn{2}{l}{\textcolor{azureCF}{\bfseries{\large L3}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l3-read-span-co'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l3-theory-span-co'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multicolumn{2}{l}{\textcolor{azureCF}{\bfseries{\large L4}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l4-read-span-co'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l4-theory-span-co'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multicolumn{2}{l}{\textcolor{azureCF}{\bfseries{\large L5}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l5-read-span-co'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l5-theory-span-co'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '    \end{tabularx}'."\n";
                        $body .= '\end{center}'."\n";
                    } else {
                ...
                when (4) {
                    ...
                    if ($calibration->{calib_multipoint}) {
                        ...
                        $body .= '         &  & \bfseries{Span letto} & \bfseries{Span teorico} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        # $body .= '        \multicolumn{2}{l}{\multirow{2}*{\textcolor{azureCF}{\bfseries{\large 100}}}} & \mdseries{'.controlnull($calib_json->{'l1-read-span-o3'}).'} & \mdseries{'.controlnull($calib_json->{'l1-theory-span-o3'}).'} \\\\'."\n";
                        $body .= '        \multicolumn{2}{c}{\textcolor{azureCF}{\bfseries{\large 100}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l1-read-span-o3'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l1-theory-span-o3'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multicolumn{2}{c}{\textcolor{azureCF}{\bfseries{\large 200}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l2-read-span-o3'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l2-theory-span-o3'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multicolumn{2}{c}{\textcolor{azureCF}{\bfseries{\large 300}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l3-read-span-o3'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l3-theory-span-o3'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multicolumn{2}{c}{\textcolor{azureCF}{\bfseries{\large 400}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l4-read-span-o3'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l4-theory-span-o3'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multicolumn{2}{c}{\textcolor{azureCF}{\bfseries{\large 500}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l5-read-span-o3'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l5-theory-span-o3'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '    \end{tabularx}'."\n";
                        $body .= '\end{center}'."\n";
                    } else {
                ...
                when (5) {
                    ...
                    if ($calibration->{calib_multipoint}) {
                        ...
                        $body .= '         &  & \bfseries{Span letto} & \bfseries{Span teorico} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        # $body .= '        \multicolumn{2}{l}{\multirow{2}*{\textcolor{azureCF}{\bfseries{\large 100}}}} & \mdseries{'.controlnull($calib_json->{'l1-read-span-btx'}).'} & \mdseries{'.controlnull($calib_json->{'l1-theory-span-btx'}).'} \\\\'."\n";
                        $body .= '        \multicolumn{2}{c}{\textcolor{azureCF}{\bfseries{\large L1}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l1-read-span-btx'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l1-theory-span-btx'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multicolumn{2}{c}{\textcolor{azureCF}{\bfseries{\large L2}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l2-read-span-btx'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l2-theory-span-btx'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multicolumn{2}{c}{\textcolor{azureCF}{\bfseries{\large L3}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l3-read-span-btx'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l3-theory-span-btx'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multicolumn{2}{c}{\textcolor{azureCF}{\bfseries{\large L4}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l4-read-span-btx'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l4-theory-span-btx'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '        \multicolumn{2}{c}{\textcolor{azureCF}{\bfseries{\large L5}}} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l5-read-span-btx'}), controlnull($calibration->{instr_type_unit})).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'l5-theory-span-btx'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                        $body .= '        \arrayrulecolor{lines}\hline'."\n";
                        $body .= '    \end{tabularx}'."\n";
                        $body .= '\end{center}'."\n";
                    } else {
                ...
                when ([14, 15, 18]) {
                    ...
                    $body .= '        \arrayrulecolor{lines}\hline'."\n";
                    $body .= '        \bfseries{Canale misurato:} & \mdseries{'.check_data_unit(controlnull($calib_json->{'read-spa-aerosol'}), controlnull($calibration->{instr_type_unit})).'} & \bfseries{Canale riferimento:} & \mdseries{'.check_data_unit(controlnull($calib_json->{'theory-span-aerosol'}), controlnull($calibration->{instr_type_unit})).'} \\\\'."\n";
                    $body .= '        \arrayrulecolor{lines}\hline'."\n";
                    $body .= '    \end{tabularx}'."\n";
                    $body .= '\end{center}'."\n";
                    $body .= "\n";
                ...
                when (25) {
                    ...
                    $body .= '        \arrayrulecolor{lines}\hline'."\n";
                    $body .= '        \bfseries{Zero trovato:} & \mdseries{'.check_data_unit(controlnull($calib_json->{'o2-zero-biogas'}), latex_encode("%")).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'ch4-zero-biogas'}), latex_encode(controlnull($calibration->{instr_type_unit}))).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'co2-zero-biogas'}), latex_encode("%")).'} \\\\'."\n";
                    $body .= '        \arrayrulecolor{lines}\hline'."\n";
                    $body .= '    \end{tabularx}'."\n";
                    $body .= '\end{center}'."\n";
                    $body .= "\n";
                    ...
                    $body .= '         & \bfseries{O2} & \bfseries{CH4} & \bfseries{CO2} \\\\'."\n";
                    $body .= '        \arrayrulecolor{lines}\hline'."\n";
                    $body .= '        \bfseries{Span letto:} & \mdseries{'.check_data_unit(controlnull($calib_json->{'read-o2-span-biogas'}), latex_encode("%")).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'read-ch4-span-biogas'}), latex_encode(controlnull($calibration->{instr_type_unit}))).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'read-co2-span-biogas'}), latex_encode("%")).'} \\\\'."\n";
                    $body .= '        \arrayrulecolor{lines}\hline'."\n";
                    $body .= '        \bfseries{Span teorico:} & \mdseries{'.check_data_unit(controlnull($calib_json->{'theory-o2-span-biogas'}), latex_encode("%")).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'theory-ch4-span-biogas'}), latex_encode(controlnull($calibration->{instr_type_unit}))).'} & \mdseries{'.check_data_unit(controlnull($calib_json->{'theory-co2-span-biogas'}), latex_encode("%")).'} \\\\'."\n";
                    $body .= '        \arrayrulecolor{lines}\hline'."\n";
                    $body .= '    \end{tabularx}'."\n";
                    $body .= '\end{center}'."\n";
                }
            ...
            if (!$arr_allegati) {
                ...
                $body .= '\vspace{0.5cm} \\\\'."\n";
                # inserisce hyperlink che rimanda alla pagina iniziale
                $body .= '\hyperlink{Manutenzione_'.$reportid.'}{\underline{Torna alla manutenzione}}'."\n";
            } else {
            ...
            # get element postion
            my $calibration_idx = firstidx { $_ eq $calibid } @calib_ids;
            $calibration_idx++; # 0 excluded
            $log->info("POSIZIONE TARATURA: $calibration_idx");
        }
    }
}
...
# WRAPPER
#------------------------------------------------------------------------------
$log->debug('wrapper manutenzione cumulativo ...');
...
# PDF FILENAME
#------------------------------------------------------------------------------
my $job_name = 'report-manutenzione-{'.$user_id.$net.'}-'.$format_from.'-'.$format_to;
