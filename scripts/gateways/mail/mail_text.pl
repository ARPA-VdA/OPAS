...
#------------------------------------------------------------------------------
#
#   Script Name : mail_text.pl
#        Author : Ecometer s.n.c.
#          Date : 2024-09-30
#   Description : Manage a text email queue
#
#   RUN EXAMPLE :
#       $ perl /path/to/script/mail_text.pl
#
#------------------------------------------------------------------------------
#------------------------------------------------------------------------------
...
use Encode;
    ...
    my $location = 'OPAS';
    my $logfile  = 'text-email.gateway.log';
    my $version  = '1.0';
    ...
    $sql = qq{ LOCK TABLE gateways.mails IN ACCESS EXCLUSIVE MODE };
    dbh_execute_query($sql);

    # TEST --> verified !
    #$log->debug("Sleep 10"); sleep(50); exit();
    ...
    $sql = qq{
        SELECT
            id, recipients, subject, body, sent_tries
        FROM
            gateways.mails
        WHERE
    ...
    if ( scalar(@$mails) ) {
        ...
        foreach my $mail ( @$mails )
        {
            ...
            if ( ! $mail->{sms_sent} ) {
                ...
                # MAIL FOOTER
                #--------------------------------------------------------
                $body .= "email inviata automaticamente @ $mail_time\n";
                ...
                if ( send_gmail_html('text/plain', undef, undef, $mail->{subject}, $body, @mail_recipients) ) {
